...
appControllers.controller('menuCtrl', function($scope, $http, $rootScope, $timeout, $mdUtil, $mdSidenav, $log, $ionicHistory, $state, $ionicPlatform, $mdDialog, $mdBottomSheet, $mdMenu, $mdSelect, $auth, $cordovaSQLite) {
      ...
      if (response == 'messagerie') {
        ...
        for (i = 0; i < response.length; i++) {
          if ($rootScope.pnjs.indexOf(response[i].pnj.__KEY) == -1) {
            $rootScope.pnjs[response[i].pnj.__KEY] = {
              'ID': response[i].pnj.__KEY,
              'fullName': response[i].pnjName,
              'subtitle': '',
              'whiteframe': 'md-whiteframe-1dp'
            };
          }
